...
class Test
{
	...
	protected function decorateAction($action)
	{
		return sprintf('<td class="action">%s</td>', $action);
	}

	protected function decorateTest($test)
	{
		return sprintf('<td class="test">%s</td>', $test);
	}

	protected function decorateExpectedVal($value = '')
	{
	...
	protected function decorateActualVal($value = '')
	{
		if ( ! $value and $this->outputAsHtml)
		{
			$value = '&nbsp;';
	...
	protected function decorateResult($test_result)
	{
		return sprintf('<td class="%s">%s</td>', strtolower($test_result), $test_result);
	}

	protected function decorateParams($params = '')
	{
	...
	protected function decorateOutput($action, $output, $test, $expected, $test_result, $params = null)
	{
		if ($test_result)
		{
			$test_result = 'PASS';
		...
		if ($this->outputAsHtml)
		{
			...
			$this->decorateTest($test) .
			$this->decorateExpectedVal($expected) .
			$this->decorateActualVal($output) .
			$this->decorateResult($test_result);


			if (is_array($params))
			{
				$message .= $this->decorateParams(print_r($params, true));
			}
			else
			{
				$message .= $this->decorateParams($params);
			}
...
//			}
	...
	public function isEqual($action, $actualResult, $expectedResult, $strict = false)
	{
		...
		return $this->decorateOutput($action, $actualResult, 'IS EQUAL TO', $expectedResult, $pass);
	}
	...
	public function fileExists($action, $filename)
	{
		...
		return $this->decorateOutput($action, ($pass ? 'True' : 'False'), 'FILE EXISTS', 'True', $pass, $filename);
	}
	...
	public function fileNotFound($action, $filename)
	{
		...
		return $this->decorateOutput($action, ($pass ? 'True' : 'False'), 'FILE NOT FOUND', 'True', $pass, $filename);
	}
	...
	public function fileHasContent($action, $filename, $expectedContent)
	{
		...
		return $this->decorateOutput($action, $actualContent, 'FILE HAS CONTENT', $expectedContent, $pass, $filename);
	}
